...
int main()
{
    ...
    App mainApp;
    Menu cont("Menu", VMenu, 100, 0, 0, texture.getSize().x * 5 + 400, texture.getSize().y * 5 + 400, sf::Color::Green);
    for (int i = 0; i < 5; i++) {
        Menu* m = new Menu("Submenu " + std::to_string(i), HMenu, 100, 0, 0, texture.getSize().x * 5 + 400, texture.getSize().y, sf::Color::Red);
        cont.addChild(m);
        for (int j = 0; j < 5; j++) {
            m->addChild(new Button("Button cont " + std::to_string(j), 0, 100 * i, texture.getSize().x, 
                          texture.getSize().y, sf::Color::White, sf::Color::Red, &texture,
                          &texture, 0, 0, 0, "hah", &font, 20,
                          sf::Color::Red, Center));
